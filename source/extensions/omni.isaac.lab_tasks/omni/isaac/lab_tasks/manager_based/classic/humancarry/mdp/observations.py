...
def object_lin_vel_rel_b(env: ManagerBasedEnv, object_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    ...
    obj: RigidObject = env.scene[object_name]
    asset: Articulation = env.scene[asset_cfg.name]
    # print(f"object_v_z: {obj.data.root_lin_vel_w[:,2]}")
...
def target_pos_rel(env: ManagerBasedRLEnv, command_name: str, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """ Object target position in the robot frame.
    """
    asset: Articulation = env.scene[asset_cfg.name]
