...
import omni.isaac.lab_tasks.manager_based.classic.go2run.mdp as mdp
...
_go2_init_pos = (0.0, 0.0, 0.4)

@configclass
class MySceneCfg(InteractiveSceneCfg):
    """Configuration for the terrain scene with a Go2 robot running."""
...
class RewardsCfg:
    ...
    rew_progress = RewTerm(func=mdp.forward_speed, weight=4.0, params={"target_vel": 8.0})
    # (2) Stay alive bonus
    rew_alive = RewTerm(func=mdp.is_alive, weight=0.2)
    # (3) Reward for maintaining desired orientation
    rew_orientation = RewTerm(func=mdp.keep_orientation, weight=0.2, params={"target_quat": math_utils.quat_inv(torch.tensor((1.0,0,0,0))).unsqueeze(0)})
    # (4) Reward for maintaining desired height
    # rew_height = RewTerm(func=mdp.keep_height, weight=1.0, params={"target_height": 0.25})
    # TODO: encourage shoulder joint pos close to 0

    # (5) Penalty for large action commands
    action_l2 = RewTerm(func=mdp.action_l2, weight=-0.001)

    # penalty for moving in y direction
    cost_off_track = RewTerm(func=mdp.off_track, weight=-0.5)

    # penalty for jumping
    jump_up = RewTerm(func=mdp.jump_up, weight=-0.5)

    # some energy related penalties
    # lin_vel_z_l2 = RewTerm(func=mdp.lin_vel_z_l2, weight=-2.0)
    cost_dof_torque = RewTerm(func=mdp.joint_torques_l2, weight=-1.0e-6)
    cost_dof_acc = RewTerm(func=mdp.joint_acc_l2, weight=-1.0e-8)
    cost_action_rate = RewTerm(func=mdp.action_rate_l2, weight=-0.0005)
    
    # energy penalty
    cost_energy = RewTerm(func=mdp.energy_consumption, weight=-1e-7)
...
class TerminationsCfg:
    ...
    torso_height = DoneTerm(func=mdp.fall_off_beam, params={"minimum_height": 0.25})
...
@configclass
class Go2runEnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for the Unitree Go2 running environment."""
