...
def bad_orientation_quat(
    env: ManagerBasedRLEnv, limit_angle_diff: float, target_quat,
...
def bad_orientation_quat_feet(
    env: ManagerBasedRLEnv, limit_angle_diff: float, target_quat, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    """Terminate when the asset's orientation is too far from the desired orientation limits.
    This is computed by checking the normalized angle eulers' difference.
    """
    ...
    quat_diff_left = math_utils.quat_mul(target_quat, quat_left_foot)
    quat_diff_right = math_utils.quat_mul(target_quat, quat_right_foot)
