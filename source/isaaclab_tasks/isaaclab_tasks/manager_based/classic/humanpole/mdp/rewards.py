...
def body_part_near_xy(
     env: ManagerBasedRLEnv, pole_xy: tuple[float,float], body_part: str, distance_limit: float, asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")
) -> torch.Tensor:
    """reward for positioning around pole xy position."""
    asset: Articulation = env.scene[asset_cfg.name]
    ind_body = asset.data.body_names.index(body_part)
    body_part_xy = asset.data.body_pos_w[:,ind_body,:2] - env.scene.env_origins[:,:2]
    dist = torch.norm(body_part_xy-torch.tensor(pole_xy,device=env.device), dim=-1)
    result = torch.clamp(-2.0 * dist + 1.0 + 2.0*distance_limit, max=1.0)
    return result