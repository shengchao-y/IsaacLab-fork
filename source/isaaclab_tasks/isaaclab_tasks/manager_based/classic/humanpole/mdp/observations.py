...
if TYPE_CHECKING:
    from isaaclab.envs import ManagerBasedEnv as BaseEnv
...
def base_dist_pole(env: BaseEnv, pole_xy: tuple[float, float], asset_cfg: SceneEntityCfg = SceneEntityCfg("robot")) -> torch.Tensor:
    """xy distance from root to pole."""
    # extract the used quantities (to enable type-hinting)
    asset: Articulation = env.scene[asset_cfg.name]
    root_xy = (asset.data.root_pos_w - env.scene.env_origins)[:, :2]
    return torch.norm(root_xy - torch.tensor(pole_xy, device=env.device), dim=-1, keepdim=True)