...
import isaaclab_tasks.manager_based.classic.humanpole.mdp as mdp
import isaaclab.utils.math as math_utils
import torch
import math

from isaaclab_assets import HUMANOID_CFG  # isort: skip
...
_pole_height = 10.0
_pole_radius = 0.2
_pole_init_pos = (0.4, 0, _pole_height/2)
...
class ObservationsCfg:
    ...
    class PolicyCfg(ObsGroup):
        ...
        # distance of torso to pole
        base_2_pole = ObsTerm(func=mdp.base_dist_pole, params={"pole_xy": _pole_init_pos[:2]})
        ...
        feet_body_forces = ObsTerm(
            ...
            scale=0.01,
            params={"asset_cfg": SceneEntityCfg("robot", body_names=["left_shin", "right_shin", "left_lower_arm", "right_lower_arm"])},
        )
...
class RewardsCfg:
    ...
    # (4) Reward for keeping body parts near pole
    left_foot_near_pole = RewTerm(func=mdp.body_part_near_xy, weight=0.05, params={"pole_xy": _pole_init_pos[:2], 
                                                                         "body_part": "left_foot",
                                                                         "distance_limit": 0.2+_pole_radius})
    right_foot_near_pole = RewTerm(func=mdp.body_part_near_xy, weight=0.05, params={"pole_xy": _pole_init_pos[:2], 
                                                                         "body_part": "right_foot",
                                                                         "distance_limit": 0.2+_pole_radius})
    left_hand_near_pole = RewTerm(func=mdp.body_part_near_xy, weight=0.05, params={"pole_xy": _pole_init_pos[:2], 
                                                                         "body_part": "left_hand",
                                                                         "distance_limit": 0.1+_pole_radius})
    right_hand_near_pole = RewTerm(func=mdp.body_part_near_xy, weight=0.05, params={"pole_xy": _pole_init_pos[:2], 
...
class TerminationsCfg:
    ...
    # terminate if hands too far away from pole
    left_hand_off = DoneTerm(func=mdp.body_part_off_pole, params={"pole_xy": _pole_init_pos[:2], 
                                                                "body_part": "left_hand",
                                                                "distance_limit": 0.25+_pole_radius})
    right_hand_off = DoneTerm(func=mdp.body_part_off_pole, params={"pole_xy": _pole_init_pos[:2], 
                                                                "body_part": "right_hand",
                                                                "distance_limit": 0.25+_pole_radius})
    # terminate if body too near to pole
    ...
    #                                                             "body_part": "pelvis",
    #                                                             "distance_limit": 0.25})
    # terminate if feet too far away from pole
    left_foot_off = DoneTerm(func=mdp.body_part_off_pole, params={"pole_xy": _pole_init_pos[:2], 
                                                                "body_part": "left_foot",
                                                                "distance_limit": 0.45+_pole_radius})
    right_foot_off = DoneTerm(func=mdp.body_part_off_pole, params={"pole_xy": _pole_init_pos[:2], 
                                                                "body_part": "right_foot",
                                                                "distance_limit": 0.45+_pole_radius})
...
@configclass
class HumanpoleEnvCfg(ManagerBasedRLEnvCfg):
    """Configuration for the MuJoCo-style Humanoid climbing pole environment."""
